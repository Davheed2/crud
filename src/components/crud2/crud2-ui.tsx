...
export function Crud2Create() {
  const { initialize } = useCrud2Program()
      ...
      {accounts.isLoading ? (
        ...
        <div className="grid md:grid-cols-2 gap-4">
          {accounts.data?.map((account) => (
            <Crud2Card key={account.publicKey.toString()} account={account.publicKey} />
          ))}
        </div>
      ) : (
...
function Crud2Card({ account }: { account: PublicKey }) {
  const { accountQuery, incrementMutation, setMutation, decrementMutation, closeMutation } = useCrud2ProgramAccount({
    account,
  })
